...
class TestListBundles:
    async def test_default_filters_committed_only(
    ...
    async def test_include_orphans_returns_both(
        self, backend: FilesystemBackend
    ) -> None:
        await _commit(backend, "store/a", [("f.txt", b"12")])
        writer = await backend.open_bundle_write("store/pending")
    ...
    async def test_mtime_populated(
        self, backend: FilesystemBackend
    ) -> None:
...
class TestReadBundleMeta:
    ...
    async def test_partial_bundle_raises(
        self, backend: FilesystemBackend
    ) -> None:
...
class TestOpenBundleFileRead:
    async def test_reads_inner_file(
        self, backend: FilesystemBackend
    ) -> None:
        await _commit(backend, "store/inner", [
...
class TestDeleteBundle:
    async def test_deletes_entire_directory(
        self, backend: FilesystemBackend, storage_dir: str
    ) -> None:
    ...
    async def test_raises_when_absent(
        self, backend: FilesystemBackend
    ) -> None:
        with pytest.raises(FileNotFoundError):
...
class TestMetaOnDisk:
    async def test_meta_json_roundtrip(
        self, backend: FilesystemBackend, storage_dir: str
    ) -> None:
        await _commit(backend, "store/rt", [("f.txt", b"hi")])
        on_disk = json.loads(
            (Path(storage_dir) / "data" / "store" / "rt" / "meta.json").read_text()
        )
