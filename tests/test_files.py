...
class TestPutFile:
    async def test_upload_returns_metadata(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_overwrite_returns_200(self, client: httpx.AsyncClient) -> None:
        await client.put("/files/report.txt", content=b"v1")
        response = await client.put("/files/report.txt", content=b"v2 updated")
        assert response.status_code == 200
        assert response.json()["size"] == len(b"v2 updated")

    async def test_upload_nested_path(self, client: httpx.AsyncClient) -> None:
        response = await client.put("/files/a/b/c/deep.txt", content=b"nested")
        assert response.status_code == 201
    ...
    async def test_rejects_disallowed_chars(self, client: httpx.AsyncClient) -> None:
        response = await client.put("/files/bad<name>.txt", content=b"data")
        assert response.status_code == 400
        assert "characters" in response.json()["detail"].lower()

    async def test_rejects_oversized_file(self, client: httpx.AsyncClient) -> None:
...
class TestGetFile:
    async def test_download_returns_content(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_download_missing_returns_404(self, client: httpx.AsyncClient) -> None:
        response = await client.get("/files/missing.txt")
        assert response.status_code == 404

    async def test_download_after_overwrite(self, client: httpx.AsyncClient) -> None:
...
class TestListFiles:
    async def test_list_returns_metadata(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_list_with_prefix(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_foreign_file_returns_404(self, client: httpx.AsyncClient, storage_dir: str) -> None:
...
class TestDeleteFile:
    async def test_delete_removes_file(self, client: httpx.AsyncClient) -> None:
